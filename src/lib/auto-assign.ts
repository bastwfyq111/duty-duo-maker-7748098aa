...
/** Detect rest-shift code (zero hours). Returns the first zero-hour code among selected codes. */  
function pickRestCode(c: AutoAssignConstraints, shifts: Record<string, ShiftType>): string {  
  const restCodes = c.shiftCodes.filter(code => (shifts[code]?.hours ?? 0) === 0);  
  return c.restCode ?? restCodes[0] ?? "";  
}  
...
function autoAssignRandomHorizontal(  
  ...
  for (let day = 1; day <= daysInMonth; day++) {  
    ...
    for (const i of order) {  
      ...
      // ✨ قاعدة: إذا كانت الخانة 1 وردية ليلية (N) فالخانة 2 راحة (R) إجبارياً  
      if (isNightCode(pick1, shifts)) {  
        emp.attendance[k2] = restCode;  
        continue;  
      }  
...
export function autoAssign(  
  ...
  // كود الراحة المستخدم لتطبيق قاعدة "ليل في الخانة 1 → راحة في الخانة 2"  
  const restForNightRule = pickRestCode(c, shifts);  
  ...
  for (let day = 1; day <= daysInMonth; day++) {  
    ...
    for (const empIdx of empOrder) {  
      ...
      // ✨ قاعدة: إذا كانت الخانة 1 ليلية (N) فالخانة 2 راحة (R) إجبارياً — تخطَّ ملء الخانة 2  
      const slot1Now = getSlot(emp, day, 1);  
      const forceRestSlot2 = isNightCode(slot1Now, shifts);  
      // for each slot try to fill if empty  
      for (const slot of [1, 2] as const) {  
        if (slot === 2 && forceRestSlot2) continue; // لا تملأ الخانة 2 عند ليلة بالخانة 1  
    ...
    // ✨ قاعدة نهائية لليوم: إذا كانت الخانة 1 وردية ليلية (N) اجعل الخانة 2 راحة (R) إجبارياً  
    if (restForNightRule) {  
      for (const emp of next) {  
        const s1 = getSlot(emp, day, 1);  
        if (isNightCode(s1, shifts)) {  
          emp.attendance[slotKey(day, 2)] = restForNightRule;  
        }  
      }  
    }  
